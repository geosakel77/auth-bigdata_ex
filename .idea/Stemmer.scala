/**
  ...
  *
  * Use the stemLine function or the stemWord.
  *
  * stemWord: returns the stem of the word, as a lowercase String. No control checks applied.
  *
  * stemLine: tokenizes the string (tokenizer is everything except for the letters of latin alphabet),
  *         then returns a Seq[String] (a Sequence of type String, which can be casted to List) with all the stems.
  *         Also disregards words that have only one letter (e.g. "a").
  */
  ...
  */
  ...
  def processSubList( l:List[(String, String)], checker: Int=>Boolean ): Boolean =
  {
    ...
    */
    var done = false
    for(tuple <-l){
      done = replacer( tuple._1, tuple._2, checker )
      if (done){
        return done
      }
    }
  ...
  def stemWordVerbose(word: String): String={
    print(word)
  ...
  def stemLine(line: String): Seq[String] ={

    var results = scala.collection.mutable.MutableList[String]()

    var word = new String

    for(c <- line.trim.toLowerCase.toCharArray){
      if(c >= 'a'&& c <= 'z'){
        ...
        //println(stemWord(word)
        if(word.length>1){
          results +=  stemWord(word)
        }
        word=""
      }
  ...
  def stemLineVerbose(line: String): Seq[String] = {
    var results = scala.collection.mutable.MutableList[String]()
...
object runit
{
  ...
  def main(args: Array[String]) =
  {
    ...
    var word = "one word and worked"
    var stemmer = new Stemmer
    stemmer

    println(stemmer.stemLine(word))
